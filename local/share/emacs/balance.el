...
(defvar balance-transaction-types
   (list
      ...
      "web"                ;; Initiated via web-banking services
      "pos"                ;; Point of sale purchase (Publix, Mobil).
      "visa"               ;; VISA credit or check card transaction
      "mc"                 ;; MasterCard credit or check card transaction
...
(if balance-mode-map
   ...
   (define-key balance-mode-map [Sh-tab]   'balance-backward-field))
...
numeric region of a transaction, based on the defined input columns."
   (let* ((amount (abs amount))
         ...
         (gap (- width2 len))
         (value (concat "%" (int-to-string len) ".2f"))
         (format-string (concat
			 "%-" (int-to-string width1) "s" value "%"
			 (int-to-string gap) "s" value)))

...
(defsubst ack-read (regexp)
  (let* ((default (if (use-region-p) 
                    ...
                    (word-at-point)))
         (pattern (if (and regexp default) (regexp-quote default) default))
         (prompt (format (if regexp "ack pattern%s: " "ack literal search%s: ")
                         (if pattern (format " (default %s)" pattern) ""))))
    (read-string prompt
                 nil
